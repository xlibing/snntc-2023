...
C_MODULE_ROOT = '/data/ccc/custom-op/snntc/'
...
def init_weight(in_size, out_size, bias_volt=1.0, init_param=15.0):
...
class SNN_dense(tf.keras.layers.Layer):
    ...
    def __init__(self, in_size, out_size, biasvolt=1.0, MAX_SPIKE_TIME=1e5, Epsilon=1e-10,
        ...
        if weight is None:
            weight = init_weight(self.in_size, self.out_size, self.biasvolt)
        else:
...
class SNN_conv(tf.keras.layers.Layer):
    ...
    def __init__(self, in_channel=1, out_channel=1, kernel_sizes=1, strides=1, padding='SAME', rates=1,
        ...
        if weight is None:
            weight = init_weight(self.in_size, self.Kc, self.biasvolt)
        else:
            weight = np.array(weight).astype(np.float32)
