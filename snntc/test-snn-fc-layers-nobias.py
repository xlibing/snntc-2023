#!/usr/bin/env python3
"""
Test C++ SNN-TC dense(fully-connected) layer module with a multiple layer model.
...
if New_Rand_Data:  # use new random parameters
    M, N, K, P = np.random.randint(1,1000,4)
    ...
    w_rand2 = np.random.randn(K, P).astype(np.float32)
    y_rand = np.random.randn(M, P).astype(np.float32)
...
My_SNN_FC_Layer = SNN_dense(in_size=N, out_size=K, weight=w_rand, biasvolt=Bias)
My_SNN_FC_Layer2 = SNN_dense(in_size=K, out_size=P, weight=w_rand2, biasvolt=Bias)
...
################### SNN tf reference model ##########################
print('start tf version ...................................')
from SNN_TC_pyref import SNNLayer
layer_in = SNNLayer(N, K, w_rand, bias=Bias)
layer_in2 = SNNLayer(K, P, w_rand2, bias=Bias)
t1 = time.time()
