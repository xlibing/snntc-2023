# train SNN with Temporal Coding over MNIST dataset
import numpy as np
...
plt.plot(np.array(train_loss)/maxnorm, 'b--', label=f'train loss:max={int(np.array(train_loss).max())}')
plt.plot(train_acc, 'r--', label='train acc')
plt.plot(np.array(test_loss)/maxnorm, 'b-', label=f'test loss:max={int(np.array(test_loss).max())}')
