...
class SNNLayer(object):
    def __init__(self, in_size, out_size, w=None, bias=False):
        ...
        if w is None:
            if self.bias:
                ...
                self.weight = tf.Variable(tf.random_uniform([self.in_size, self.out_size], 0. / self.in_size,
                                                                       8. / self.in_size, tf.float32))
        else:
...
class SCNNLayer(object):
    def __init__(self, kernel_size=3, in_channel=1, out_channel=1, strides=1, w=None, bias=False):
        self.MAX_SPIKE_TIME = 1e5
        ...
        self.kernel = SNNLayer(in_size=self.kernel_size * self.kernel_size * self.in_channel,
                                        out_size=self.out_channel, w=w, bias=bias)
